...
static const int gpsRX = 3, gpsTX = 2;
...
String ipAddress = "203.175.11.123:8080"; // this is the ip address of our server - e.g. "123.123.123.123"
String APN = "byu";                       // check your Internet provider's website for the APN e.g. "internet"
...
// variables for a well-scheduled loop - in which the sendData() gets called every 30 secs (refresh rate)
unsigned long last;
...
void sendLocation(String lat, String lon)
{
    ...
    Sim800l.println("AT+HTTPPARA=\"URL\", \"http://" + ipAddress + "/chat-url/" + lat + "/" + lon + "\"");
    waitUntilResponse("OK");
    // GO
